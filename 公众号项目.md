...
2. 调用 chooseImage，返回一个promise，调用微信接口（wx.chooseImage）
   ...
   ![](/Users/mac/Desktop/笔记/images/chooseImage成功返回数据.png)

   ![](/Users/mac/Desktop/笔记/images/wx.chooseImage.png)

   file对象结构为

   ![](/Users/mac/Desktop/笔记/images/file对象结构.png)
...
3. 然后使用getLocalImgData，参数为ids，也就是前面的localIds获取本地图片。这个getLocalImgData会调用wx.getLocalImgData接口使用，返回一个promise，然后根据返回的结果进行拼接成base64格式
   ...
   ![](/Users/mac/Desktop/笔记/images/imageBase65.png)
...
4. 然后根据这个base64数据，进行分析
   ...
   ![](/Users/mac/Desktop/笔记/images/blob.png)
...
6. 父组件上绑定的afterRead方法需要传入两个参数，一个是file，一个是name。如果flie为数组，也就是传入多个图片，就需要拆分单独赋值。
   ...
   ![](/Users/mac/Desktop/笔记/images/图片压缩返回的blob对象.png)
   ...
   ![](/Users/mac/Desktop/笔记/images/图片的url.png)
